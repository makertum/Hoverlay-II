...
float col_length=total_length/cols;
//float top_row_width=2*(top_width)/(rows+1);
float top_row_width=top_width/rows;
// float bottom_row_width=2*(bottom_width)/(rows+1);
float bottom_row_width=bottom_width/rows;
...
String file_name="supercomb_fog.gcode";
...
void addLayer(float z) {
  ...
  for (float y=0;y<rows;y++) {
    for (float x=0;x<cols;x++) {
      rectangle(
      bed_x_offset  +0.5*col_length  +x*col_length, 
      bed_y_offset  +y*layer_row_width+layer_col_offset, 
      z, 
      col_length, 
      layer_row_width,
      x==0, 
...
void rectangle(float x, float y, float z, float w, float h, boolean firstX, boolean firstY, boolean lastX, boolean even) {
